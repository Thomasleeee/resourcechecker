...
public class ResourceChecker : EditorWindow {
	...
	void OnGUI ()
	{
        ...
        if (GUILayout.Button("sort", GUILayout.Width(80), GUILayout.Height(20)))
        {
            Debug.Log("sort");
        }
		...
		{			
           ...
           // tex.GetType().GetProperty("SRGB")
            ...
            if (tDetails.npot)
                ...
                sizeLabel += "-isnot npot\n";
            if (tDetails.issRGB)
                sizeLabel += "-is sRGB";
            else
                sizeLabel += "-is not sRGB";
    ...
    private TextureDetails GetTextureDetail(Texture tTexture)
	{
		...
		if (tTextureDetails == null)
		{
            ...
            tTextureDetails.issRGB = (tTexture as RenderTexture).sRGB;
		}
